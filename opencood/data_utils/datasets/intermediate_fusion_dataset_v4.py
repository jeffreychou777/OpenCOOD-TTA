...
class IntermediateFusionDatasetV4(basedataset.BaseDataset):
    """
    This class is for intermediate fusion with unsupervised pre-training
    where we generate two data streams: teacher and student
    """

    def __init__(self, params, visualize, train=True):
        super(IntermediateFusionDatasetV4, self).__init__(params, visualize, train)

        self.pre_processor_q = build_preprocessor(params["preprocess_q"], train)
        self.pre_processor_k = build_preprocessor(params["preprocess_k"], train)
        self.post_processor = post_processor.build_postprocessor(
            params["postprocess"], train
        )

        self.is_augment = params["fusion"]["args"]["is_augment"]
        self.ssl=params["fusion"]["args"]["ssl"]
    ...
    def __getitem__(self, idx):
        ...
        scenario_path=base_data_dict[ego_id]['scenario_path']
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            if cav_id == ego_id:
                projected_lidar_ego = selected_cav_processed["projected_lidar"]
            else:
        ...
        if self.ssl or len(projected_lidar_stack) == 0:
            projected_lidar_stack.append(projected_lidar_ego)
            # convert list to numpy array, (N, 4)
            projected_lidar = np.vstack(projected_lidar_stack)
        else:
            projected_lidar = np.vstack(projected_lidar_stack)
            projected_lidar_ego, projected_lidar = projected_lidar, projected_lidar_ego
        ...
        projected_lidar = mask_points_by_range(
            projected_lidar, self.params["preprocess_q"]["cav_lidar_range"]
        )
        projected_lidar_ego = mask_points_by_range(
            projected_lidar_ego, self.params["preprocess_k"]["cav_lidar_range"]
        )

        if self.ssl:
            scenario_folder=scenario_path.split('/')[-2]
            scenario_timeStamp=scenario_path.split('/')[-1]
            seg_path=f'segmanet/{scenario_folder}/{scenario_timeStamp}'
            seg_folder=f'segmanet/{scenario_folder}'
            if os.path.isfile(os.path.join(seg_path + ".seg")):  
        ...
        processed_lidar_q = self.pre_processor_q.preprocess(projected_lidar)
        processed_lidar_k = self.pre_processor_k.preprocess(projected_lidar_ego)
        ...
        processed_data_dict["ego"].update(
            {
                ...
                "processed_lidar_q": processed_lidar_q,
                "processed_lidar_k": processed_lidar_k,
        ...
        if self.visualize:
            processed_data_dict["ego"].update({"origin_lidar": projected_lidar})
            if self.ssl:
                processed_data_dict["ego"].update({'segments':segments})
    ...
    def collate_batch_train(self, batch):
        ...
        object_bbx_mask = []
        object_ids = []
        processed_lidar_q_list = []
        processed_lidar_k_list = []
        ...
        for i in range(len(batch)):
            ...
            processed_lidar_q_list.append(ego_dict["processed_lidar_q"])
            processed_lidar_k_list.append(ego_dict["processed_lidar_k"])
            ...
            if self.visualize:
                ...
                if self.ssl:
                    segments.append(ego_dict['segments'])
        ...
        # np.array([3,5,6]), ...]}
        merged_feature_q_dict = self.merge_features_to_dict(processed_lidar_q_list)
        merged_feature_k_dict = self.merge_features_to_dict(processed_lidar_k_list)
        processed_lidar_q_torch_dict = self.pre_processor_q.collate_batch(
            merged_feature_q_dict
        )
        processed_lidar_k_torch_dict = self.pre_processor_k.collate_batch(
            merged_feature_k_dict
        )
        # [2, 3, 4, ..., M], M <= max_cav
        record_len = torch.from_numpy(np.array(record_len, dtype=int))
        
        if self.ssl:
            label_torch_dict = self.post_processor.collate_batch_v2(label_dict_list)
        else:
        ...
        output_dict["ego"].update(
            {
                ...
                "processed_lidar_q": processed_lidar_q_torch_dict,
                "processed_lidar_k": processed_lidar_k_torch_dict,
        ...
        if self.visualize:
            ...
            if self.ssl:
                ...
                output_dict["ego"].update({'segments': segments})
    ...
    @staticmethod
    def transform_to_bounding_boxes(input_array):
        unique_groups = np.unique(input_array[:, 4][input_array[:, 4] != -1])
        ...
        for group in unique_groups:
            # 选择属于当前组的点云
            group_points = input_array[input_array[:, 4] == group][:, :4]


            # 计算当前组的最大最小范围
            min_x_coords = np.min(group_points[:,0])
            max_x_coords = np.max(group_points[:,0])
            min_y_coords = np.min(group_points[:,1])
            max_y_coords = np.max(group_points[:,1])

            # 将最大最小范围与组号合并为一组数据
            bounding_box_data = np.array([min_x_coords,min_y_coords, max_x_coords, max_y_coords])
            # 添加到输出数组
            output_array.append(bounding_box_data)
